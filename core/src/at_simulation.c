    ...
    for (uint32_t i = 0; i < total_rays; i++) {
        ...
        while (ray->energy > MIN_ENERGY_THRESHOLD) {
            if (bvh) {
                ...
                child = (AT_Ray *)malloc(sizeof(AT_Ray));
                ...
                *child = ctx.out_ray;
                ...
                AT_Ray closest = AT_ray_init((AT_Vec3){{FLT_MAX, FLT_MAX, FLT_MAX}}, (AT_Vec3){{0, 0, 0}}, 0.0f, 0.0f, i);
                bool intersects = false;
                uint32_t tri_idx = 0;
                AT_Vec3 normal;
                for (uint32_t t = 0; t < simulation->scene->environment->index_count / 3; t++) {
                    AT_Triangle *triangles = simulation->scene->triangle_arrs->triangles_db;
